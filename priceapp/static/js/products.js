...
function logout() {
  ...
  if (confirm("Are you sure you want to logout?")) {
    ...
    if (
      ...
      if (
        !confirm(
          "You have unsaved products. Are you sure you want to logout and lose this data?"
        )
      ) {
        return;
      }
...
function previewPhoto(input) {
  ...
  if (file) {
    ...
    reader.onload = function (e) {
      ...
      if (
        existingPreview &&
        !existingPreview.getAttribute("data-remove-photo")
      ) {
        existingPreview.style.opacity = "0.5";
        const infoDiv = existingPreview.querySelector(
...
function addPriceRow(productId, sizeId) {
  ...
  priceRow.innerHTML = `
        ...
        <td>${createInput(
          ...
          "",
          "form-control",
          'placeholder="Price" oninput="updatePriceCalc(this)"'
        )}</td>
        <td>
          <div>${createInput(
            "number",
            "",
            "form-control",
            'placeholder="%" oninput="updatePriceCalc(this)"'
          )}</div>
          <small class="text-muted">₹${createInput(
            "number",
            "",
  ...
  const targetSizeRow = Array.from(document.querySelectorAll(`tr`)).find(
    (row) =>
  ...
  if (targetSizeRow) {
    ...
    const allRelatedRows = Array.from(document.querySelectorAll(`tr`)).filter(
      (row) =>
    ...
    const insertAfter =
      allRelatedRows.length > 0
        ? allRelatedRows[allRelatedRows.length - 1]
        : targetSizeRow;
...
function addSizeRow(productId) {
  ...
  const allProductRelatedRows = Array.from(
    document.querySelectorAll(`tr`)
  ).filter(
    (row) =>
      row.classList.toString().includes(`product_${productId}`) &&
      row.id !== `product_${productId}`
  );

  if (allProductRelatedRows.length > 0) {
    // Insert after the last related row
    const lastRelatedRow =
      allProductRelatedRows[allProductRelatedRows.length - 1];
...
function updatePriceCalc(element) {
  ...
  if (boxQty > 0) {
    ...
    if (boxDiscountPriceInput)
      boxDiscountPriceInput.value = boxPriceAfterDiscount.toFixed(2);
...
function updateDealerCalc(element) {
  ...
  const purchaseDiscountPercent = parseFloat(
    dealerDiscountPercentInput?.value || 0
  );
  const purchaseTaxPercent = parseFloat(dealerTaxPercentInput?.value || 0);

  const purchaseDiscountAmount = parseFloat(
    calc(purchasePrice, purchaseDiscountPercent)
  );
  const purchasePriceAfterDiscount = purchasePrice - purchaseDiscountAmount;
  const purchaseTaxAmount = parseFloat(
    calc(purchasePriceAfterDiscount, purchaseTaxPercent)
  );
  ...
  const purchaseBoxDiscountPercent = parseFloat(
    dealerBoxDiscountPercentInput?.value || 0
  );
  const purchaseBoxTaxPercent = parseFloat(
    dealerBoxTaxPercentInput?.value || 0
  );

  if (purchaseBoxQty > 0) {
    const purchaseBoxPrice = purchasePrice * purchaseBoxQty;
    const purchaseBoxDiscountAmount = parseFloat(
      calc(purchaseBoxPrice, purchaseBoxDiscountPercent)
    );
    const purchaseBoxPriceAfterDiscount =
      purchaseBoxPrice - purchaseBoxDiscountAmount;
    const purchaseBoxTaxAmount = parseFloat(
      calc(purchaseBoxPriceAfterDiscount, purchaseBoxTaxPercent)
    );
    const purchaseBoxFinalTotal =
      purchaseBoxPriceAfterDiscount + purchaseBoxTaxAmount;

    // Show final box price after discount
    if (dealerBoxDiscountPriceInput)
      dealerBoxDiscountPriceInput.value =
        purchaseBoxPriceAfterDiscount.toFixed(2);
    // Show final box price after tax
    if (dealerBoxTaxPriceInput)
      dealerBoxTaxPriceInput.value = purchaseBoxFinalTotal.toFixed(2);
  }
}
...
document.getElementById("saveProductsBtn").addEventListener("click", () => {
  ...
  productRows.forEach((pRow) => {
    ...
    if (productData.photo && typeof productData.photo !== "object") {
      console.error(
        `❌ Invalid photo data type: ${typeof productData.photo}`,
        productData.photo
      );
    }
    ...
    sizeRows.forEach((sizeRow) => {
      ...
      const priceRows = document.querySelectorAll(
        `.product_${productId}_size_${sizeId}_price`
      );
      priceRows.forEach((priceRow) => {
        ...
        dealerRows.forEach((dealerRow) => {
          ...
          const dealerData = {
            ...
            purchase_date: safeDate(dealerInputs[2]?.value),
...
async function saveAllProducts(data) {
  ...
  try {
    ...
    if (errorCount > 0) {
      console.log(
        `⚠️ ${errorCount} products failed to save. Check console for details.`
      );
    }
  } catch (err) {
...
function cancelEdit() {
  ...
  document.getElementById("saveProductsBtn").className =
    ...
    sizes.forEach((size) => {
      ...
      if (prices.length === 0) {
        ...
        prices.forEach((price) => {
          totalRows += Math.max(
            (price.dealers && price.dealers.length) || 1,
            1
          );
        });
      }
...
function populateFormWithProductData(product) {
  ...
  document.getElementById("saveProductsBtn").style =
  ...
  if (product.sizes && product.sizes.length > 0) {
    product.sizes.forEach((size, sizeIndex) => {
      ...
      const currentSizeRow = document.querySelector(
        `.product_0_size[data-size-id="${actualSizeId}"]`
      );
      ...
      if (currentSizeRow) {
        ...
        if (size.prices && size.prices.length > 0) {
          size.prices.forEach((price, priceIndex) => {
            ...
            const currentPriceRow = document.querySelector(
              `.product_0_size_${actualSizeId}_price[data-price-id="${actualPriceId}"]`
            );
            ...
            if (currentPriceRow) {
              ...
              if (paymentSelect)
                paymentSelect.value = price.payment_type || "cash";
              ...
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.price || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.discount || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.discount_price || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.tax || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.tax_price || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.box || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.box_discount || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.box_discount_price || "";
              inputIndex++;
              if (inputs[inputIndex])
                inputs[inputIndex].value = price.box_tax || "";
              inputIndex++;
              if (inputs[inputIndex])
